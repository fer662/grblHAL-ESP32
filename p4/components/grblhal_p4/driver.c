...
bool p4_motion_idle(void) {return !running && !pulse_phase;}
...
static void IRAM_ATTR steps_write(uint8_t mask)
{
    ...
    gpio_ll_set_level(&GPIO, P4_X_STEP, !!(mask & X_AXIS_BIT));
    gpio_ll_set_level(&GPIO, P4_Z_STEP, !!(mask & Z_AXIS_BIT));
}
...
{
    direction = 0;
    gpio_ll_set_level(&GPIO, P4_X_DIR, direction_invert.x);
    gpio_ll_set_level(&GPIO, P4_Z_DIR, direction_invert.z);
}
...
void IRAM_ATTR p4_motion_fault(void)
{
    fault = true;
    idle(true);
...
static void IRAM_ATTR schedule_pulse(uint32_t ticks)
{
    ...
    if (gptimer_set_alarm_action(pulse_timer, &alarm) != ESP_OK) p4_motion_fault();
}
static void IRAM_ATTR assert_pulse(void)
{
    ...
    if (pending_steps.x) {
        ...
        diag.x_pulses++;
        diag.x_position += (gpio_ll_get_level(&GPIO, P4_X_DIR) ^ direction_invert.x) ? -1 : 1;
    }
    if (pending_steps.z) {
...
static void IRAM_ATTR pulse_start(stepper_t *stepper)
{
    ...
    if (direction_changed) {
        ...
        gpio_ll_set_level(&GPIO, P4_X_DIR, !!(output & X_AXIS_BIT));
        gpio_ll_set_level(&GPIO, P4_Z_DIR, !!(output & Z_AXIS_BIT));
    }
...
static void IRAM_ATTR cycles(uint32_t ticks)
{
    ...
    if (ticks < 250) { p4_motion_fault(); return; } // 40 kHz ISR ceiling
    if (ticks > P4_STEP_HZ * 2) ticks = P4_STEP_HZ * 2;
    uint64_t now;
    ...
    if (gptimer_set_alarm_action(step_timer, &alarm) != ESP_OK) p4_motion_fault();
}
static bool IRAM_ATTR step_alarm(gptimer_handle_t timer, const gptimer_alarm_event_data_t *event, void *ctx)
{
    ...
    diag.interrupts++;
    p4_fpu_call(hal.stepper.interrupt_callback);
...
static void delay_ms(uint32_t ms, delay_callback_ptr callback)
{
    ...
    if (callback) {
        if (!ms) delayed(NULL);
        else if (!task_add_delayed(delayed, NULL, ms)) p4_motion_fault();
...
{
    ESP_ERROR_CHECK(gpio_set_level(P4_X_ENABLE, P4_X_ENABLE_OFF));
    ESP_ERROR_CHECK(gpio_set_level(P4_Z_ENABLE, P4_Z_ENABLE_OFF));
    gpio_config_t outputs = {.pin_bit_mask = (1ULL << P4_X_ENABLE) | (1ULL << P4_Z_ENABLE), .mode = GPIO_MODE_INPUT_OUTPUT};
    ESP_ERROR_CHECK(gpio_config(&outputs));
    esp_ldo_channel_config_t ldo = {.chan_id = P4_IO_LDO_CHANNEL, .voltage_mv = P4_IO_LDO_MV};
    ESP_ERROR_CHECK(esp_ldo_acquire_channel(&ldo, &io_ldo));
    outputs.pin_bit_mask = (1ULL << P4_X_STEP) | (1ULL << P4_Z_STEP) | (1ULL << P4_X_DIR) | (1ULL << P4_Z_DIR);
    outputs.mode = GPIO_MODE_INPUT_OUTPUT;
    steps_write(0);
    gpio_set_level(P4_X_DIR, direction_invert.x);
