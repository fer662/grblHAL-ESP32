...
int main() {
    ...
    assert(h5_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Sync/run-up: Z 0.005 to 0.020 mm"));
    assert(strstr(text,"Full-depth thread: Z 0.905 to 9.100 mm | length 8.195 mm"));
    assert(strstr(text,"X depth: first 1.100, final 2.000 mm | After Z stops: X 0.500 mm"));
    assert(!strstr(text,"(est.)") && !strstr(text,"Usable thread"));
    measure=1;format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Sync/run-up: Z 0.00020 to 0.00079 in"));
    assert(strstr(text,"Full-depth thread: Z 0.03563 to 0.35827 in | length 0.32264 in"));
    ...
    assert(h5_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Before Z starts: X to -0.001 mm (surface 0.000)"));
    assert(strstr(text,"Final-pass X infeed: Z 0.020 to 1.145 mm"));
    assert(strstr(text,"Full-depth thread: Z 1.145 to 8.860 mm | length 7.715 mm"));
    assert(strstr(text,"Final-pass withdrawal: Z 8.860 to 9.985 mm | Z stop: 10.000 mm"));
    assert(strstr(text,"X depth: first 0.200, final 1.000 mm | After Z stops: X -0.500 mm"));
    c.pitch=-.5;assert(h5_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Full-depth thread: Z 8.855 to 1.140 mm | length 7.715 mm"));
    ...
    assert(h5_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Full-depth thread: Z 2.020 to 7.985 mm | length 5.965 mm"));
    assert(strstr(text,"Final-pass withdrawal: Z 7.985 to 9.880 mm | Z stop: 10.000 mm"));
    m.x_max_rate=300;m.x_acceleration=500;
    assert(h5_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
    assert(strstr(text,"Full-depth thread: Z 1.145 to 8.860 mm | length 7.715 mm"));
    assert(strstr(text,"Final-pass withdrawal: Z 8.860 to 9.880 mm | Z stop: 10.000 mm"));
